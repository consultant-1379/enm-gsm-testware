...
#     JIRA        : NO Jira
#
#     Description : updating Code base for new GSM node names
#
#     Date        : 06th Apr 2021
#
#####################################################################################
...
#     JIRA        : NSS-33841
#
#     Description : Ciphers Support for BSC node
#
#     Date        : 23rd Dec 2020
#
####################################################################################
...
NodesList=`echo -e ".open $SIMNAME \n .show simnes" | /netsim/inst/netsim_shell | grep -v ">>" | grep -v "OK" | grep -v "NE" | cut -d" " -f1`
for Node in ${NodesList[@]}
do
if [[ $Node == *"B"* && $Node == *"MSRBS"* ]] || [[ $Node == *"B"* && $Node == *"M"* && $Node != *"BSP"* && $Node != *"IPLB"* ]];
then
echo "****$Node****"
cat >> $Node.mo << DEF
SET
